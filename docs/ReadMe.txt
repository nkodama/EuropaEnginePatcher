Europa Engine Patcher Ver 0.44
...
■使用方法
  ...
  4. オプションの「自動命名時の語順変更」については、和訳作者様の指示に従って下さい。
     わからなければチェックをオフのままでお願いします。
  5. 開始→保存の順にボタンを押すと、
     保存ダイアログが開きますので、適当なファイル名で保存して下さい。
  6. 添付の_inmm.dllをゲームフォルダへコピーして下さい。
     本バージョンでパッチを当てた実行ファイルは、
     TObject様製の_inmm.dllと互換性がありません。
  7. 日本語化ファイル一式をゲームフォルダへ上書きコピーし、ゲームを開始して下さい。
...
■制約事項
  1. Steam版には対応していません。(解決不能)
     ...
     DH1.02などゲームの種類によっては動作するものがありますが、
     作者が検証できない以上サポート外とします。
     Steam版の日本語化方法に関する質問には答えられません。
  2. チャットウィンドウで5ChまたはA7hを含む文字(暴/噂/十/申/表/予など)を使うと化けます。(未調査)
